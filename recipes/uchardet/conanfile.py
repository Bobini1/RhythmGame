...
class UchardetConan(ConanFile):
    ...
    def _patch_sources(self):
        ...
        # fix broken cmake
        tools.replace_in_file(self, os.path.join(self._source_subfolder, "CMakeLists.txt"),
                              "${CMAKE_BINARY_DIR}",
    ...
    def _configure_cmake(self):
        ...
        tc.variables["BUILD_STATIC"] = False  # disable building static libraries when self.options.shared is True
        cmake = CMake(self)
    ...
    def package(self):
        shutil.copy(self._source_subfolder + "/COPYING", "licenses")
        cmake = self._configure_cmake()
        cmake.install()
        tools.rmdir(self, os.path.join(self.package_folder, "lib", "pkgconfig"))
        tools.rmdir(self, os.path.join(self.package_folder, "share"))

    def package_info(self):
        self.cpp_info.set_property("cmake_file_name", "uchardet")
        self.cpp_info.set_property("cmake_target_name", "uchardet::uchardet")
