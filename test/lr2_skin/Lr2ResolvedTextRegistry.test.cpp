...
TEST_CASE("LR2 resolved text registry refreshes queued active ids", "[lr2][text]") {
	...
	QJSEngine engine;
	QJSValue resolver = engine.evaluate(QStringLiteral("(function(id) { return 'text-' + id; })"));
...
TEST_CASE("LR2 resolved text registry full refresh uses active text ids", "[lr2][text]") {
	ensureCoreApplication();
	...
	QJSEngine engine;
	QJSValue resolver = engine.evaluate(QStringLiteral("(function(id) { return 'all-' + id; })"));
