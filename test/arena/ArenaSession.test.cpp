...
auto
withRoundMemberFields(QJsonObject result,
...
phase2Member(QString memberId,
             ...
             qint64 availabilityAppliedRevision = 0) -> QJsonObject
{
    return withRoundMemberFields(
      member(std::move(memberId), std::move(displayName)),
...
{
    if (members.isEmpty()) {
        members.append(phase2Member(selfMemberId, QStringLiteral("Alice")));
    } else {
          ...
          "[arena][session][rounds]")
{
    ...
    QObject::connect(&fixture.session,
                     ...
                     [&](gameplay_logic::ChartRunner* value) {
                         ...
                         if (value != nullptr) {
                             // Simulate expensive synchronous QML creation. The
                             // release deadline must remain anchored to message
                             // receipt.
    ...
    SECTION("scheduled resume retains and rearms the same runner")
    {
        ...
        resumed.insert(QStringLiteral("members"),
                       QJsonArray{ phase2Member(QStringLiteral("member-1"),
                                                QStringLiteral("Alice"),
                                                QStringLiteral("ready"),
                                                6,
    ...
    SECTION("pre-start cancellation pops before destroying the runner")
    {
        ...
        CHECK(fixture.roundLoader.cancellations == QVector<quint64>{ loadId });
          ...
          "[arena][session][rounds]")
{
    ...
    QObject::connect(&fixture.session, &arena::ArenaSession::roundChanged, [&] {
        if (fixture.session.getRoomPhase() == arena::RoomPhase::Selecting) {
          ...
          "[arena][session][rounds]")
{
    ...
    round.insert(QStringLiteral("participants"),
                 QJsonArray{ QJsonObject{
          ...
          "[arena][session][chat][rejoin]")
{
    ...
    REQUIRE(
      chat->data(chat->index(0, 0), arena::ArenaChatModel::SelfRole).toBool());
    ...
    fixture.session.joinRoom(QStringLiteral("room-1"), QString{});
    const auto join = messageObject(fixture.transport.textCalls.back().message);
    REQUIRE(join.value(QStringLiteral("type")).toString() ==
            QStringLiteral("room_join"));
    const auto currentMessage = chatMessage(QStringLiteral("message-new-self"),
                                            QStringLiteral("member-2"),
                                            QStringLiteral("Alice"),
    ...
    fixture.transport.injectText(
      ...
      roomSnapshot(
        ...
        roomSnapshotData(QStringLiteral("seat-token-2"),
                         4,
                         2,
    ...
    CHECK(
      chat->data(chat->index(0, 0), arena::ArenaChatModel::SelfRole).toBool());
    CHECK(
      chat->data(chat->index(1, 0), arena::ArenaChatModel::SelfRole).toBool());
}
