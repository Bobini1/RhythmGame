...
set(python_args PYTHON_VERSION "3")
if(CMAKE_HOST_WIN32)
    set(python3_handoff_file "${CURRENT_PORT_DIR}/../../.vcpkg-ci-python3")
    if(EXISTS "${python3_handoff_file}")
        file(READ "${python3_handoff_file}" python3_from_ci)
        string(STRIP "${python3_from_ci}" python3_from_ci)
        file(TO_CMAKE_PATH "${python3_from_ci}" python3_from_ci)
    elseif(DEFINED ENV{VCPKG_PYTHON3})
        file(TO_CMAKE_PATH "$ENV{VCPKG_PYTHON3}" python3_from_ci)
    elseif(DEFINED ENV{pythonLocation})
        file(TO_CMAKE_PATH "$ENV{pythonLocation}/python.exe" python3_from_ci)
    endif()
    if(DEFINED python3_from_ci AND EXISTS "${python3_from_ci}")
        message(STATUS "Using CI Python for qtinterfaceframework: ${python3_from_ci}")
        set(python_args PYTHON_VERSION "3" PYTHON_EXECUTABLE "${python3_from_ci}")
    endif()
endif()

x_vcpkg_get_python_packages(${python_args}
