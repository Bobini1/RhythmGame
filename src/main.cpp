...
main(int argc, [[maybe_unused]] char* argv[]) -> int
{
    ...
    auto db = db::SqliteCppDb{
        support::pathToQString((assetsFolder / "song_db.sqlite")).toStdString()
    };
    ...
    auto profileList = qml_components::ProfileList{ &db,
                                                    availableThemes,
    ...
    auto hitRulesFactory =
      [](gameplay_logic::rules::TimingWindows timingWindows,
         std::function<double(std::chrono::nanoseconds)> hitValuesFactory) {
          return std::make_unique<gameplay_logic::rules::StandardBmsHitRules>(
    ...
    auto scanningQueue = qml_components::ScanningQueue{ &db, songDbScanner };
    ...
    qmlRegisterSingletonInstance(
      "RhythmGameQml", 1, 0, "RootSongFoldersConfig", &rootSongFoldersConfig);

    auto songFolderFactory = qml_components::SongFolderFactory{ &db };
    qmlRegisterSingletonInstance(
      "RhythmGameQml", 1, 0, "SongFolderFactory", &songFolderFactory);

    auto previewFilePathFetcher = qml_components::PreviewFilePathFetcher{ &db };
    qmlRegisterSingletonInstance(
      "RhythmGameQml", 1, 0, "PreviewFilePathFetcher", &previewFilePathFetcher);
    ...
    engine.addImageProvider("ini", new resource_managers::IniImageProvider{});
